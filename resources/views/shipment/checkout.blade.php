    ...
    <div class="card shadow-lg border-0">
        <div class="card-header bg-primary text-white">
            <div class="row">
                <div class="col-md-11">
                    <h5 class="mb-0">
                        <i class="fas fa-truck me-2"></i> Check Out Shipment
                    </h5>
                </div>
                <div class="col-md-1">
        ...
        @endif
            ...
            @if(!empty($shipmentdetails->checkout))
                <div class="alert alert-secondary">
                    <i class="fas fa-sticky-note me-2"></i>
                    <strong>Sudah Check-Out : </strong> {{ $shipmentdetails->checkout }}
                </div>
            @else
            <div class="alert alert-info d-flex align-items-center mt-3">
                <i class="fas fa-info-circle me-2"></i>
                <div>Pastikan semua data sudah sesuai sebelum melakukan <strong>Check Out</strong>.</div>
            </div>
            @endif


            {{-- Tombol Aksi --}}
            <form action="{{ route('do.checkout.store', $shipment->noshipment) }}" method="POST" class="mt-4">
                @csrf
                <div class="d-flex justify-content-end gap-2">
                    ...
                    @else
                        ...
                        <button type="submit" class="btn btn-success">
                            <i class="fas fa-check-circle me-1"></i> Konfirmasi Check-Out
                        </button>
                    @endif
                </div>
            </form>
